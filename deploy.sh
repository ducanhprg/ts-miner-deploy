...
sshpass -p $password ssh -o 'StrictHostKeyChecking=no' $username@$remote_server_ip -p $port "sudo apt install git & git clone https://github.com/ducanhprg/ts-miner-deploy.git miner"
echo "RUNNING MINER ..."
sshpass -p $password ssh -o 'StrictHostKeyChecking=no' $username@$remote_server_ip -p $port "cd miner & bash miner.sh -a $pool -b $wallet -c $worker & disown & nvidia-nmi"
